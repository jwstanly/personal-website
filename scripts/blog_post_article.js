...
async function publishArticle() {
  ...
  const currentArticle = await fetch(`https://api.${DOMAIN_NAME}/blog?title=${title.split(" ").join("+")}`, {
    method: 'GET',
    headers: {
  ...
  const subheader = input(`Subheader${currentArticle ? ` [${currentArticle.subheader}]` : ''}: `);
  const image = input(`Image${currentArticle ? ` [${currentArticle.image}]` : ''}: `);
  const tags = input(`Tags (split by \`${TAG_DELIMITER}\`)${currentArticle ? ` [${currentArticle.tags.join(TAG_DELIMITER)}]` : ''}: `);
  ...
  await fetch(`https://api.${DOMAIN_NAME}/blog?title=${title.split(" ").join("+")}`, {
      headers: {
        'Accept': 'application/json',
        'X-API-KEY': API_KEY,
        'Content-Type': 'application/json',
