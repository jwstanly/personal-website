...
function getStackStatus(stackStdout) {
  ...
  const endIndex = stackStdout.substring(SesCfStack.indexOf('StackStatus')).indexOf('\n');
...
if (!sh.which('aws') || !sh.which('sam')) {
  sh.echo('You must install both the AWS CLI and the AWS Serverless Application Model (SAM) CLI to run this script');
  sh.exit(1);
}

if (!sh.which('git')) {
  sh.echo('You must install git to clone the repo for making custom resources for AWS SES');
  sh.exit(1);
}
