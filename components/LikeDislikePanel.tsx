...
export function LikeDislikePanel(props: LikeDislikePanelProps){
  ...
  function setVotes() {
    let tempLikes = 0;
    let tempDislikes = 0;
    props.article && props.article.votes && props.article.votes.map(vote => {
  ...
  async function onVote(newVote: VoteType) {
    ...
    if (newVote === "LIKE" && (!userCurrentVote || userCurrentVote === "NEUTRAL")) {
      setLikes(likes+1);
      newUserVote = "LIKE";
    } else if (newVote === "DISLIKE" && (!userCurrentVote || userCurrentVote === "NEUTRAL")) {
